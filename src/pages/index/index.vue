<template>
  <view class="pb-1">
    <Lingzhi v-for="(item,index) in list" :key="item.id" :detail="item" :img-list="item.imgList" @clickDynamic="clickDynamic(index)" @clickUser="clickUser(item.id)" />
    <u-calendar v-model="calendarShow" :mode="mode" @change="changePublishTime" />
    <u-modal v-model="inputShow" title="请输入新的内容" @confirm="changeContent">
      <view class="p-4 slot-content">
        <u-input v-model="newContent" :auto-height="true" :border="true" :height="100" placeholder="请输入..." type="textarea" />
      </view>
    </u-modal>
    <u-modal v-model="removeShow" content="确认删除这条记录吗?" show-cancel-button @confirm="removeItem" />
    <u-action-sheet v-model="actionListShow" :list="actionList" @click="handleActionClick" />
...
export default {
  ...
  methods: {
    handleActionClick(index) {
      const actionMap = ['calendarShow', 'inputShow', 'removeShow']
